    ...
    columns: [
      ...
      },
      {
        defaultContent:
          ...
          "<div class='text-center'><div class='btn-group'><button class='btn btn-primary btn-sm btnEditar'><i class='material-icons'>edit</i></button></div></div>",
      },
    ],
  });
...
/*=============================   INICIO DE CRUD DE LAS TABLAS  ================================= */
  ...
  $("#idni").blur(function () {
    //Consulta de disponibilidad de Cédula al cambiar el click
    idni = $("#idni").val();
    if (idni.length == 0) {
      $("#Aviso").text("Ingrese el Número de Cédula").css("color", "red");
    } else {
      if (idni.length == 8) {
        opcion = 5;
        $.ajax({
          ...
          success: function (response) {
            ...
            switch (response) {
              ...
              case "1":
                $("#Aviso").text("Cédula ya está registrado").css("color", "red");
                break;
              case "2":
                $("#Aviso").text("Cédula no registrado").css("color", "green");
                break;
              default:
        ...
        $("#Aviso").text("La Cédula debe tener 8 dígitos.").css("color", "red");
      }
    }
  });

  // $("#idni").keypress(function(e) { //Consulta de disponibilidad de Cédula al dar enter
  //   if(e.which == 13) {
  //     e.preventDefault();
  //     idni = $('#idni').val();
  //     if(idni.length == 0){
  ...
  $(document).on("click", ".btnEditfoto", function () {
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        $('#iddni1').val(idni);
        $('#idusua').val(user_id);
        $("#FotoP").attr("src","/sismedoc/" + data[0]["foto"]);
...
$("#FormFotop").on('submit', function(e){
  ...
  $(document).on("click", ".btnDerivar", function () {
    fila = $(this).closest("tr");
    id = fila.find("td:eq(0)").text(); //capturo el Nro expediente
    dni = fila.find("td:eq(3)").text(); //capturo el Cédula DEL remitente
    $("#exp1").val(id);
    $("#dnir").val(dni);
    $("#nrodesc").text(id);
  ...
  $("#NuevoPDF").click(function () {
    $("#NuevoPDF").attr("href", "/sismedoc/" + archi);
  });
  ...
  $("#btnBusca").click(function () {
    ...
    if (id.length < 6 ) {  
      alert("El N° de Expediente debe tener 6 dígitos");
    } else {
      $.ajax({
        url: "../../controller/cruddocumento.php",
        type: "POST",
...
// VALIDACION DEL Cédula INGRESADO EN MESA DE ENTRADA
  $("#validar").click(function () {
    ...
    var idni = $("#iddni").val();
    if(idni.length < 8){
      alert('La Cédula debe tener 8 digitos');
      $("#iddni").focus();
    }else{
      $.ajax({
      url: "../../controller/crudusu.php",
    ...
    if(tipo == "0"){
      ...
      if(tipo == "3" && (desde="" || hasta == "")){
        ...
        $("#start").focus();
          ...
          window.open('/sismedoc/reporte/reporte-documentos.php?e='+estado+'&a='+año+'&m='+mes+'&d='+d+'&h='+h, '_blank');
      }
    } 
  });
...
function salir() {
  window.location = "/sismedoc/controller/salir.php"; //pagina donde tienes tus consultas para borrar
}
